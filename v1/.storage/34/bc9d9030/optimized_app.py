...
def init_database():
    ...
    # Create analytics table for tracking changes
    conn.execute('''
...
class SmartDataManager:
    def __init__(self):
        ...
        self.previous_player_data = {}  # Track previous states for analytics
    ...
    def get_analytics_data(self, time_range='24h'):
        ...
        # Get kill trends
        cursor = self.db.execute('''
            SELECT datetime(timestamp, 'localtime') as hour, COUNT(*) as kills
            FROM analytics_log 
            WHERE event_type = 'kill' AND timestamp > ?
            GROUP BY strftime('%Y-%m-%d %H', timestamp)
            ORDER BY hour
        ''', (since.isoformat(),))
        
        kills_data = [{'time': row[0], 'value': row[1]} for row in cursor.fetchall()]
        
        # Get shot trends
        cursor = self.db.execute('''
            SELECT datetime(timestamp, 'localtime') as hour, SUM(CAST(new_value as INTEGER) - CAST(old_value as INTEGER)) as shots
            FROM analytics_log 
            WHERE event_type = 'shot' AND timestamp > ?
            GROUP BY strftime('%Y-%m-%d %H', timestamp)
            ORDER BY hour
        ''', (since.isoformat(),))
        
        shots_data = [{'time': row[0], 'value': row[1] if row[1] else 0} for row in cursor.fetchall()]
        
        # Get recent activities
        cursor = self.db.execute('''
            SELECT username, event_type, old_value, new_value, family, 
        ...
        for row in cursor.fetchall():
            username, event_type, old_val, new_val, family, timestamp = row
            
            if event_type == 'kill':
                activities.append({
                    'type': 'kill',
                    'player': username,
                    'target': 'Unknown',  # We don't track targets yet
                    'family': family,
                    'time': self.format_time_ago(timestamp)
                })
            elif event_type == 'shot':
                shot_diff = int(new_val) - int(old_val) if new_val.isdigit() and old_val.isdigit() else 0
                if shot_diff > 0:
                    activities.append({
                        'type': 'shot',
                        'player': username,
...
def get_analytics():
    """API endpoint voor analytics data"""
    try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"📈 Analytics: Kill/Shot trend tracking actief")
